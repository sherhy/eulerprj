...
def straights(hand, score):
	cnums = sorted(hand)
	straight = True
	spcase = [[1,2,3,4,5],[1,2,3,4,13],[1,2,3,12,13],[1,2,11,12,13],[1,10,11,12,13]]
	# mayberoyal = False
	if score[2]: 
		straight = False
	else:
...
rounds = 0
p1wincount = 0
g = open('p054_hands.txt','w')
for line in hands:
	rounds +=1
	twohands = sorthands(line)
	aassddff = ','.join(twohands[0])+'\t'+','.join(twohands[1])
	g.write(aassddff)
	if comparehands(twohands): 
		g.write("\tP1Win",)
		p1wincount+=1
