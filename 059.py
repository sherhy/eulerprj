...
and the preferred standard is ASCII (American Standard Code 
for Information Interchange). For example, uppercase A = 65, 
...
def checknthchar(n, string):
	showstr = ""
	for k in range((len(string)-3-n)/3):
		showstr = showstr + string[n+k*3]
	print showstr
...
while True:
	...
	for num in words:
		...
		nnn = int(num)
		nmn = nnn ^ code[i%3]
		ciphered = ciphered + chr(nmn)	

	# checknthchar(2, ciphered)
